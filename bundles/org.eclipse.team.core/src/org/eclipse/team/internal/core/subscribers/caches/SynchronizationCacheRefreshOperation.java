...
public abstract class SynchronizationCacheRefreshOperation {
	...
	protected IResource[] refresh(IResource resource, int depth, boolean cacheFileContentsHint, IProgressMonitor monitor) throws TeamException {
		...
		try {
			...
			IRemoteResource	tree = getRemoteTree(resource, depth, cacheFileContentsHint, Policy.subMonitorFor(monitor, 70));
	 ...
	 */
	protected abstract IRemoteResource getRemoteTree(IResource resource, int depth, boolean cacheFileContentsHint, IProgressMonitor monitor) throws TeamException;
