...
/**
 * VestigeVestigeXMLWriter
 */
public class VestigeXMLWriter extends PrintWriter {
	protected int tab;

	/* constants */
	protected static final String XML_VERSION = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"; //$NON-NLS-1$

	public VestigeXMLWriter(OutputStream output) throws UnsupportedEncodingException {
	...
	public void write(VestigeConfigurationItem item) {
		...
		VestigeConfigurationItem[] children = item.getChildren();
		if(children != null) {
			for (int i = 0; i < children.length; i++) {
				VestigeConfigurationItem child = children[i];
				write(child);
			}
		}
	...
	static public void writeXMLPluginMetaFile(Plugin plugin, String filename, VestigeConfigurationItem element) throws TeamException {		
		...
		try {
			VestigeXMLWriter writer = new VestigeXMLWriter(new BufferedOutputStream(new FileOutputStream(tempFile)));
			try {
