...
import org.eclipse.team.internal.ui.sync.views.ISyncSetChangedListener;
import org.eclipse.team.internal.ui.sync.views.SubscriberInput;
import org.eclipse.team.internal.ui.sync.views.SyncSet;
import org.eclipse.team.internal.ui.sync.views.SyncSetChangedEvent;
 ...
 */
public class SyncViewerDirectionFilters extends SyncViewerActionGroup implements ISyncSetChangedListener {
	...
	/* (non-Javadoc)
	 * @see org.eclipse.team.internal.ui.sync.views.ISyncSetChangedListener#syncSetChanged(org.eclipse.team.internal.ui.sync.views.SyncSetChangedEvent)
	 */
	public void syncSetChanged(SyncSetChangedEvent event) {
		updateEnablement(event.getSet());
	}

	private void updateEnablement(SyncSet syncSet) {
//		for (Iterator it = actions.iterator(); it.hasNext();) {
//			DirectionFilterAction action = (DirectionFilterAction) it.next();
//			if(syncSet == null) {
//				action.setEnabled(false);	
//			} else {
//				action.setEnabled(syncSet.getCount(action.getFilter()) > 0);
//			}
//		}
	}
	...
	protected void initializeActions() {
		...
		if(input != null) {
			SyncSet ss = input.getSubscriberInputSyncSet();
			ss.addSyncSetChangedListener(this);
			updateEnablement(input.getSyncSet());
		} else {
			updateEnablement(null);
		}
		//viewer.getInput().getSyncSet().addSyncSetChangedListener(this);
	...
	public void removeContext(ActionContext context) {
		...
		if(input != null) {
			SyncSet ss = input.getSubscriberInputSyncSet();
			ss.removeSyncSetChangedListener(this);
			updateEnablement(null);
		}
	}
}