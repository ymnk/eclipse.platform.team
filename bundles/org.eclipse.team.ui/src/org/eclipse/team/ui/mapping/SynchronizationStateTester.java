 ...
 * to both resource deltas and change events from the decorated state provider
 * ({@link #getDecoratedStateProvider()}.
 * <p>
...
public class SynchronizationStateTester {
	 ...
	 * If the method returns <code>true</code>, no team state decorations should be applied
	 ...
	 * is capable of decorating. The state mask is obtained from the provider
	 * returned from {@link #getDecoratedStateProvider()}.
	 * 
	 * @param element
	 *            the model element to be decorated
	 * @return the mask that indicates what state the appropriate team decorator
	 *         will decorate
	 * @see TeamStateProvider#getDecoratedStateMask(Object)
	 */
	...
	public int getState(Object element, int stateMask, IProgressMonitor monitor) throws CoreException {
		ITeamStateDescription desc = getDecoratedStateProvider().getStateDescription(element, stateMask, new String[0], monitor);
		if (desc != null)
	 ...
	 * provider
	 */
	public ITeamStateProvider getDecoratedStateProvider() {
